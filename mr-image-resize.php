...
/**
  ...
  */
add_action('delete_attachment', 'wntr_mr_delete_resized_images');
function wntr_mr_image_resize($url, $width=null, $height=null, $crop=true, $align='c', $retina=false) {
  global $wpdb;
  // Get common vars
  $args = func_get_args();
  $common = wntr_mr_common_info($args);
...
function wntr_mr_common_info($args) {
  // Unpack arguments
  ...
  if (preg_match('/\/[0-9]{4}\/[0-9]{2}\/.+$/', $urlinfo['path'], $matches)) {
    ...
    $main_path = get_theme_root().'/Evessi/images/webi/placeholder.png';
    $file_path = preg_replace('/(\/\/)/', '/', $main_path);
  }
...
function wntr_mr_delete_resized_images($post_id) {
